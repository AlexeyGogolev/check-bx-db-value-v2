#!/bin/bash
#
# Site: https://github.com/AlexeyGogolev/check-bx-db-value-v2
#
...
DBSettings_keys=(login password database)
...
if  [ -z "${CLParams[${CLParams_keys[$param_num-1]}]}" ] ; then
    ...
    printf "Example: \n\t$(basename ${BASH_SOURCE[0]}) \"/www/ab.cd/bitrix/.settings.php\" \"select count(id) from b_event where SUCCESS_EXEC<>'Y'\" 5\n" 
    exit 10
fi
# выход если конфиг пустой или его нет 

printf "File ${CLParams[path_to_bxdb_config]} config. \n"
...
export MYSQL_PWD=${DBSettings[password]}
...
num_value=`${mysql} -u ${DBSettings[login]} -N -B -e "use ${DBSettings[database]}; ${CLParams[single_num_value_query]}"`
# отображение значений
echo "Result of the query (from DB ${DBSettings[database]}): ${num_value}, ${CLParams_keys[2]}: ${CLParams[max_num_value]}"
# сравнение значений
